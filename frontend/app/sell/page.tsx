...
const BENEFITS = [
  { icon:<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={P} strokeWidth="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>, title:"Your own online store", desc:"Set up in under 10 minutes. Add products, photos and prices — no technical skills needed." },
  ...
  { icon:<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke={P} strokeWidth="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>, title:"WhatsApp sharing built in", desc:"Share your store and products to WhatsApp with one tap — reach buyers in your contacts." },
...
const FAQS = [
  ...
  { q:"What can I sell?",                  a:"Anything legal — clothing, food, beauty, electronics, handmade crafts, services, art. If you make it or sell it, list it." },
  { q:"Do I need a registered business?",  a:"No. Individuals, sole traders and registered businesses are all welcome." },
  { q:"How does the 5% commission work?",  a:"On the Free plan we take 5% per sale. Sell a R200 item — you keep R190. Upgrade to Basic or Pro for lower commission." },
          ...
          {PLANS.map(plan => (
            <div key={plan.name} style={{ background:WHITE, borderRadius:16, padding:"18px", border:`${plan.highlight?"2.5":"1"}px solid ${plan.highlight?plan.color:BORDER}`, position:"relative" }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:4, marginBottom:14 }}>
                {plan.features.map(f => <div key={f} style={{ display:"flex", alignItems:"center", gap:8, fontSize:12, color:DARK }}><span style={{ color:plan.color, fontWeight:700 }}>✓</span>{f}</div>)}
              </div>

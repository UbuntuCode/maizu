...
const AD_BANNERS = [
  {
    ...
    sub:       "New arrivals from South African designers",
    cta:       "Shop now",
    bg:        "#1A1A2E",
    accent:    "#E8401C",
  ...
  },
  {
    id: 2,
    tag:  "FRESH IN",
    headline:  "New arrivals every day",
    sub:       "Discover products from local SA vendors",
    cta:       "Browse stores",
    bg:        "#0D1B2A",
    accent:    "#F59E0B",
  ...
  },
  {
    id: 3,
    tag:  "HANDMADE IN SA",
    headline:  "Support local crafters",
    sub:       "Unique handmade goods from Mzansi artisans",
    cta:       "Explore crafts",
    bg:        "#1B1F1B",
    accent:    "#10B981",
...
const CATEGORIES = [
  { label:"Fashion",     img:"https://images.unsplash.com/photo-1490481651871-ab68de25d43d?w=200&q=80",  q:"fashion"     },
  { label:"Electronics", img:"https://images.unsplash.com/photo-1518770660439-4636190af475?w=200&q=80",  q:"electronics" },
  { label:"Beauty",      img:"https://images.unsplash.com/photo-1522335789203-aabd1fc54bc9?w=200&q=80",  q:"beauty"      },
  { label:"Food",        img:"https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=200&q=80",  q:"food"        },
  { label:"Home",        img:"https://images.unsplash.com/photo-1555041469-a586c61ea9bc?w=200&q=80",     q:"home"        },
  { label:"Sports",      img:"https://images.unsplash.com/photo-1517649763962-0c623066013b?w=200&q=80",  q:"sports"      },
  { label:"Art & Crafts",img:"https://images.unsplash.com/photo-1513364776144-60967b0f800f?w=200&q=80",  q:"crafts"      },
  { label:"Services",    img:"https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?w=200&q=80",  q:"services"    },
];
...
const ImgPlaceholder = () => (
  <div style={{ width:"100%", height:"100%", background:"linear-gradient(135deg,#F4F4F4,#E8E8E8)", display:"flex", alignItems:"center", justifyContent:"center" }}>
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#CACACA" strokeWidth="1.5">
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (sd.success) setStores(sd.stores || []);
      } catch { /* silent */ }
    };
    load();
  }, []);
    ...
    <div className="maizu-shell">
      ...
      <main className="page-content" style={{ paddingBottom:80 }}>
        ...
        <div style={{ padding:"14px 14px 0" }}>
          ...
          >
            <img src={banner.img} alt={banner.headline} style={{ position:"absolute", inset:0, width:"100%", height:"100%", objectFit:"cover", opacity:0.6 }} />
            ...
            <div style={{ position:"absolute", inset:0, padding:"20px 22px", display:"flex", flexDirection:"column", justifyContent:"flex-end" }}>
              ...
              <div style={{ background:banner.accent, color:"#fff", borderRadius:8, padding:"7px 16px", fontSize:12, fontWeight:700, display:"inline-block", alignSelf:"flex-start" }}>
                {banner.cta}
              </div>
            </div>
            {/* Dots */}
        ...
        <div style={{ padding:"20px 14px 0" }}>
          <SHead title="Shop by category" action="All categories" onAction={() => router.push("/stores")} />
          <div className="category-grid">
            {CATEGORIES.map(cat => (
              <button key={cat.label} onClick={() => router.push(`/search?q=${cat.q}`)}
                ...
                <div style={{ width:"100%", aspectRatio:"1", borderRadius:12, overflow:"hidden", background:"#F4F4F4" }}>
        ...
        {products.length > 0 && (
          <div style={{ padding:"22px 14px 0" }}>
            ...
            <div className="product-grid">
              {products.filter((p:any) => p.is_trending || true).slice(0, 8).map((p:any) => (
                <div key={p.id} style={{ background:T.white, borderRadius:12, overflow:"hidden", border:`0.5px solid ${T.border}`, cursor:"pointer" }}>
                  <div style={{ aspectRatio:"1", background:"#F4F4F4", overflow:"hidden", position:"relative" }}>
                    ...
                    {p.is_trending && (
                      <div style={{ position:"absolute", top:8, left:8, background:T.primary, color:"#fff", borderRadius:4, padding:"2px 7px", fontSize:9, fontWeight:700 }}>HOT</div>
                    )}
                    {p.stock_quantity === 0 && (
                      <div style={{ position:"absolute", inset:0, background:"rgba(255,255,255,0.6)", display:"flex", alignItems:"center", justifyContent:"center" }}>
                        <span style={{ fontSize:11, fontWeight:700, color:T.muted }}>Sold out</span>
                      </div>
                    )}
                  </div>
                  <div style={{ padding:"10px 10px 12px" }}>
                    <div style={{ fontSize:12, fontWeight:500, color:T.dark, lineHeight:1.35, marginBottom:3, display:"-webkit-box", WebkitLineClamp:2, WebkitBoxOrient:"vertical", overflow:"hidden" }}>
                      {p.name}
                    </div>
                    <div style={{ fontSize:11, color:T.muted, marginBottom:8 }}>{p.store_name || "Store"}</div>
        ...
        <div style={{ margin:"22px 14px 0", background:T.dark, borderRadius:16, padding:"20px 22px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:16 }}>
          <div>
            ...
            <div style={{ fontSize:12, color:"rgba(255,255,255,0.55)" }}>Free store · Accept card payments · SA vendors only</div>
          </div>
          <button onClick={() => router.push("/sell")} style={{ background:T.primary, color:"#fff", border:"none", borderRadius:9, padding:"10px 18px", fontSize:13, fontWeight:700, cursor:"pointer", flexShrink:0, whiteSpace:"nowrap" }}>
            Open store
          </button>
        </div>
            ...
            <div className="store-grid">
              {stores.slice(0, 8).map((s:any) => (
                <div key={s.id} onClick={() => router.push(`/stores/${s.id}`)} style={{ background:T.white, borderRadius:12, overflow:"hidden", border:`0.5px solid ${T.border}`, cursor:"pointer" }}>
                  <div style={{ height:80, background:s.banner_url?"transparent":`linear-gradient(135deg,${T.primary}22,${T.primary}44)`, overflow:"hidden", position:"relative" }}>
        ...
        {products.length === 0 && stores.length === 0 && (
          <div style={{ padding:"48px 20px", textAlign:"center" }}>
            ...
            <div style={{ fontSize:13, color:T.muted, marginBottom:24, maxWidth:280, margin:"0 auto 24px" }}>Be the first vendor. Open your store and start selling across South Africa.</div>

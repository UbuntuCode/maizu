    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.55)", zIndex:9999, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
      <div style={{ background:WHITE, borderRadius:16, padding:"24px 20px", width:"100%", maxWidth:380 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
          ...
          <button onClick={onClose} style={{ background:BG, border:"none", borderRadius:"50%", width:30, height:30, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>
        ...
        {preview ? (
          <div style={{ textAlign:"center" }}>
            <img src={preview} alt="" style={{ width:"100%", maxHeight:200, objectFit:"contain", borderRadius:10, marginBottom:12 }} />
            {searching && <div style={{ fontSize:13, color:P, fontWeight:600 }}>Finding similar products…</div>}
          </div>
        ) : (
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
            <button onClick={() => fileRef.current?.click()}
              ...
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={P} strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
              <span style={{ fontSize:12, fontWeight:600, color:P }}>Upload photo</span>
            </button>
            <button onClick={() => fileRef.current?.click()}
              ...
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#2563EB" strokeWidth="2"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg>
...
/* ══════════════════════════════════════════════════════════════
   HEADER — responsive, no overflow on mobile
══════════════════════════════════════════════════════════════ */
  ...
  return (
    <>
      ...
      <header className="maizu-header" style={{
        ...
        <form onSubmit={handleSearch} style={{ flex:1, minWidth:0, display:"flex", alignItems:"center", background:BG, borderRadius:9, padding:"0 10px", height:36 }}>
          ...
          />
          <button type="button" onClick={() => setShowImgSrch(true)} style={{ background:"none", border:"none", cursor:"pointer", padding:"2px 0 2px 4px", display:"flex", flexShrink:0 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:2, flexShrink:0 }}>
          ...
          <button onClick={() => router.push("/notifications")}
            ...
            <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke={DARK} strokeWidth="2">
          ...
          <button onClick={() => router.push("/cart")}
            ...
            {totalItems > 0 && (
              <span style={{ position:"absolute", top:4, right:4, background:P, color:WHITE, borderRadius:"50%", width:14, height:14, fontSize:8, display:"flex", alignItems:"center", justifyContent:"center", fontWeight:700 }}>
                {totalItems > 9 ? "9+" : totalItems}
              </span>
            )}
          </button>

          {/* Account — icon only on mobile, text on desktop */}
          <button onClick={() => router.push(isLoggedIn ? "/profile" : "/login")}
            style={{ background:"none", border:`0.5px solid ${BORDER}`, cursor:"pointer", padding:"5px 8px", borderRadius:8, display:"flex", alignItems:"center", gap:5 }}>
            <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke={DARK} strokeWidth="2">
              <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/>
              <circle cx="12" cy="7" r="4"/>
            </svg>
            {/* Name visible only on wider screens via CSS */}

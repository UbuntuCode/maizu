...
const createPayment = async (req, res) => {
  if (!PF_MERCHANT_ID || !PF_MERCHANT_KEY) {
    return res.status(503).json({
      success: false,
      message: "PayFast not configured. Add PAYFAST_MERCHANT_ID, PAYFAST_MERCHANT_KEY and PAYFAST_PASSPHRASE to Railway.",
    });
  }
  ...
  try {
    ...
    /* Build payment URL */
    var queryString = Object.keys(pfData)
...
const handleNotify = async (req, res) => {
  /* Respond 200 immediately — PayFast requires fast response */
  res.status(200).send("OK");

  try {
    var pfData       = req.body;
    var pfParamString = Object.keys(pfData)
    ...
    if (!verify.valid) {
      console.error("PayFast ITN invalid:", verify.reason);
      return;
    }
    ...
    if (paymentStatus === "COMPLETE" && order.status === "pending") {
      ...
      try {
        ...
        await sendNotification(
          ...
          "order_placed",
          "Payment confirmed! 🎉",
          `Your PayFast payment for order #${orderId.slice(0, 8).toUpperCase()} was successful.`,
          { order_id: orderId }
        );
      } catch { /* non-critical */ }
    }
...
const verifyOrder = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success:        true,
      payment_status: order.status === "confirmed" ? "COMPLETE" : order.status,
      order,
    });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
